...
- **Block rendering**: `block_render.rs` lowers a block's markdown to an IR (`BlockNode`/`InlineNode`, pure and unit-testable) and renders each **paragraph as one shaped `StyledText`** — a single string, one `TextRun` per style change, plus the byte ranges that respond to a click. Do *not* go back to a div per inline span: that made every span a shrinkable flex item, and once one had been measured narrow, gpui's text measure cache (`elements/text.rs` returns the cached size for any indefinite width query) kept re-confirming that width, so any block that had to wrap collapsed into a one-character-wide column. Consequences of the single-element model: (1) `InlineExtension` (the hook for `[[Page]]`, `#tag`, and later `((block-id))`) returns an `InlineRun` — text, colour, background, italic, an optional `Box<dyn Action>` — never an element, so a tag cannot have a border and a link cannot set a hover cursor; (2) clicks are hit-tested with `TextLayout::index_for_position` against those ranges, and mouse-*down* on a range must `stop_propagation()`, or `BlockView` swaps in the editor before the mouse-up that would complete the click; (3) every run names its colour explicitly, because a wrapper div's `text_color` is pushed during layout, *after* `render` has already built the runs — an inherited colour silently comes out as gpui's default black. `every_run_carries_a_theme_colour` pins that last one against the runs the real render path builds.
